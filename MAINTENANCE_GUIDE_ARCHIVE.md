...
### Why GPU Reads Correctly: Structural Ground vs Dedicated Wire
...
**This explains why the differential analysis shows GEA-specific degradation:** The G1000-ECU differential isolates changes that affect Pin 47 (structural ground) but NOT the ECU (dedicated wire ground). The −0.88V total degradation (p=5.4e-12) and the progressive worsening at each R&R are consistent with structural ground connections being disturbed and not properly restored each time.

**What needs to be verified:**

1. **Battery negative terminal** — the junction where structural ground (Cable 200), instrument panel ground (24008A4N), and GPU ground (24405A6N) all meet. If the stack-to-post contact is degraded, both the structural and wired paths are affected, but the GPU bypasses it.
2. **Cable 200 and its termination** — this cable connects the battery post to the structural ground network. Trace where it goes. Inspect both ends.
3. **Structural ground connections in the battery area** — bonding straps, frame joints, any metal-to-metal connections that form part of the structural ground return path.
4. **Pin 47 (wire 31299A22BL)** — trace this wire to find where it physically connects to the structural ground. That connection point is in the chain.
...
**The ECU proves the dedicated-wire GS-IP ground infrastructure is healthy:** Per AMM p1936-1937 (Drawing D44-9274-10-00, EECU Wiring), the AE300 ECU (under the pilot's seat) grounds to **GS-IP-3 and GS-IP-4** → GS-IP bus bar → wire 24008A4N (4 AWG) → battery negative. This dedicated wire path reads correctly. The structural ground path (used by Pin 47 and the alternator) is the suspect.

**Four areas to inspect (in order of priority):**

1. **Battery negative terminal, Cable 200, and structural grounds (aft fuselage)** — Per the IPC (24-31), three connections at battery negative: wire 24008A4N (instrument panel ground return), wire 24405A6N (EPU), and **Cable 200** (D44-2403-160-00, "Cable, Battery GND" — routes to structural/airframe ground). The **BatteryMinder interface** (installed Sep 2024) also connects here. This terminal has been disturbed during both engine R&Rs (Feb 2024, Jul 2025) and the battery replacement (Jul 2025). **This is the most likely location for the fault** — it's where the structural ground network meets the battery, and it's where the GPU bypass works.
   ...
   **Cable 200 (Battery GND) and structural ground inspection — CRITICAL:**
   ...
   - **This cable is the bridge between the structural ground network and the battery.** If Pin 47's generic ground returns through the structural ground system, Cable 200 is in that return path. A bad connection at either end of Cable 200 would elevate the entire structural ground relative to battery negative.
   - Inspect the termination: corrosion, loose fastener, poor contact area, paint under terminal
   - Check the cable itself for chafing, damage, or corrosion
   - **Document ALL ground connections visible in the battery area** — there may be bonding straps or ground points not shown on the IPC that are part of the structural ground network
   - Measure resistance from Cable 200's far-end termination back to battery negative post (should be < 0.005Ω)

2. **Pin 47 (ANALOG IN 5 LO) Essential Bus ground** — wire 31299A22BL (shielded) connects to the low side of the Essential Bus per the G1000 wiring diagram (D44-9231-60-03). The Electrical System schematic shows only a generic ground symbol — **the physical termination point is unknown and must be traced**. Since the GEA reads Pin 46 minus Pin 47 (differential), Pin 47 is the voltage measurement reference. Any resistance at this ground directly causes a low reading. **Pin 47's generic ground symbol likely means it connects to the structural ground** — the same network that Cable 200 serves. If so, Pin 47's return path goes: wire 31299A22BL → structural ground point → airframe structure → Cable 200 → battery negative post. Every joint in that chain is a potential failure point.
...
4. **GS-IP-14 / Pin 20 (POWER GROUND)** — wire 77016A22N. The GEA's power ground. This goes through the **dedicated wire path** (GS-IP-14 → bus bar → 24008A4N → battery negative), which is proven healthy by the ECU data. However, if Pin 20 and Pin 47 are on different ground networks (Pin 20 on the dedicated wire path, Pin 47 on the structural ground), the potential difference between them could exceed the ADC common-mode range, causing erratic readings — consistent with the high noise observed in the flight data.
...
### ECU Differential Proves the Fault Is GEA-Specific

By subtracting the ECU voltage from the G1000 voltage for each matched flight (114 pairs), we eliminate all shared factors — bus voltage changes from alternator/VR replacements, battery condition, wire 24008A4N, the GS-IP bus bar — and isolate only the GEA 71S-specific degradation.

| Period | G1000 Mean | ECU Mean | G1000 − ECU | G1000 Excess Noise |
|--------|-----------|---------|-------------|-------------------|
| Before R&R #1 (pre Feb 2024) | 27.29V | 27.61V | **−0.31V** | −0.108V (G1000 quieter) |
| Between R&Rs (Feb 2024 – Apr 2025) | 26.90V | 27.77V | **−0.88V** | +0.115V |
| After R&R #2 (Jul 2025+) | 26.90V | 28.09V | **−1.19V** | +0.248V |

**Step changes at each R&R (GEA-specific degradation only):**

| Event | G1000−ECU Step | Statistical Significance |
|-------|---------------|------------------------|
| R&R #1 (Feb 2024) | **−0.56V worse** | p = 2.2e-08 |
| R&R #2 (Jul 2025) | **−0.31V worse** | p = 6.9e-03 |
| **Total degradation** | **−0.88V** | **p = 5.4e-12** |

Both R&Rs made the GEA-specific path worse. Before R&R #1, the G1000 was actually **quieter** than the ECU (−0.108V excess noise). After R&R #2, the G1000 has **+0.248V more noise** than the ECU. This progressive worsening is consistent with a connection that gets disturbed during each maintenance visit but never properly repaired.

![ECU differential three-period analysis](output/ecu_differential_three_period.png)

**How to read this chart:**

- **Top panel** — G1000 (blue dots) and ECU (green triangles) absolute voltages. The ECU actually went UP over time (due to alternator/VR replacements improving bus output), while the G1000 went down. The gap widened at each R&R.
- **Middle panel** — The G1000−ECU differential. Green = before R&R #1 (−0.31V), orange = between R&Rs (−0.88V), red = after R&R #2 (−1.19V). Each R&R introduced a step change in the GEA-specific direction only.
- **Bottom panel** — Noise comparison. G1000 noise (blue) increases at each R&R while ECU noise (green) stays flat — the increasing noise is GEA-specific.
