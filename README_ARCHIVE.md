...
### ECU Differential Analysis (GEA-Specific Isolation)

By subtracting the ECU voltage from the G1000 voltage for each matched flight (114 same-day pairs from 184 G1000 flights and 265 ECU sessions), all shared factors are eliminated — bus voltage changes from alternator/VR replacements, battery condition, wire 24008A4N, the GS-IP bus bar. What remains is purely the GEA 71S-specific degradation.
...
#### Step Changes at Each R&R (GEA-Specific Only)
...
Both R&Rs worsened the GEA-specific path. Before R&R #1, the G1000 was actually **quieter** than the ECU. After R&R #2, the G1000 has +0.248V more noise. The ECU absolute voltage went *up* across periods (27.61 → 27.77 → 28.09V) due to alternator/VR replacements improving bus output — but the differential proves the G1000's measurement path degraded independently.
...
**Probable mechanism — battery terminal corrosion:** During the Feb 2024 engine R&R, the battery was disconnected and the ring terminals on the B1(-) post (24008A4N, 24405A6N, Cable 200) were removed and left exposed in a humid Florida hangar for the duration of the repair. The bare copper/tin contact surfaces oxidized and developed a corrosion film. When reassembled, the contact surfaces were likely not cleaned and **Dow Corning DC4 compound was not applied** (per AMM 24-31, DC4 should be applied to the battery terminal and cable lugs during reconnection). The result: a persistent oxide/corrosion layer at the ring terminal-to-post interface, creating the ~0.065Ω contact resistance that produces the 1.3V offset. Subsequent work also failed to clean the terminals: the Jul 2025 battery replacement gave a new post but the ring terminals on the wires were not cleaned (new post + old corroded lugs = still bad); the Sep 2024 BatteryMinder install added a fourth terminal without cleaning existing ones; the Jul 2025 R&R #2 disconnected and reconnected without cleaning or applying DC4. **The fix may be as simple as cleaning every contact surface to bright metal, reassembling in the correct stacking order, and applying DC4 per the AMM.**
...
### Structural Ground vs Dedicated Wire — Why the GPU Bypasses the Fault

The aircraft has **two types of ground return paths**, and they behave very differently:

1. **Dedicated wire grounds** — continuous copper wire from the component to battery negative. Examples: wire 24008A4N (4 AWG, GS-IP bus bar → battery negative), wire 24405A6N (6 AWG, GPU negative → battery negative). Low resistance, reliable.

2. **Structural/airframe grounds** — shown as generic ground symbols on the schematic. Current returns through a chain of metal-to-metal joints (frame sections, bonding straps, engine mounts) and ultimately through **Cable 200** ("Cable, Battery GND") to the battery negative post. Higher resistance, susceptible to corrosion at each joint.

**Pin 47 (ANALOG IN 5 LO)** — the GEA's voltage measurement reference — is shown with a **generic ground symbol** on the schematic. This means Pin 47 connects to the **structural ground network**.

**The alternator** is also shown with a **generic ground symbol** — it grounds through engine block → engine mount bolts → firewall → airframe structure. It too depends on the structural ground to return current to battery negative.

**The ECU** grounds through **GS-IP-3/4** → GS-IP bus bar → **wire 24008A4N** (4 AWG, dedicated copper wire) → battery negative. It never touches the structural ground.

```
DEDICATED WIRE PATH (healthy — ECU uses this and reads correctly):
...
Alt    → engine block → engine mount → firewall → structure → Cable 200 → Battery B1(-)
```

If the structural ground path has resistance — at any joint in the chain, at Cable 200's termination, or at the battery post where the ring terminal stack contacts the post — then:

**Battery only:** All return current (~20A) must cross the degraded structural ground path. Pin 47 floats above true battery negative. 20A × 0.065Ω = **1.3V drop**.

**GPU connected:** The GPU cable (24405A6N) is a **dedicated 6 AWG copper wire** to the same battery negative terminal stack. It provides a parallel low-impedance return. Current preferentially flows through the GPU wire instead of the degraded structural ground. Pin 47's reference is pulled down toward the GPU's clean ground.

**In flight (alternator):** The alternator grounds through the **same structural ground** as Pin 47. No GPU wire is connected (EPU plug is open circuit). No bypass. Higher current (~25A+), worse drop.
...
|-----------|----------------------|-------------------|-----------------|
| Battery only, ground | Structural → Cable 200 → post | **No** | **-1.3V** |
| GPU connected | Structural **OR** GPU wire (24405A6N) | **Yes** — GPU wire shunts current | **-0.19V** |
| Flight (alternator) | Structural → Cable 200 → post | **No** — alt also on structural ground | **-1.4V avg** |
...
1. **Battery negative terminal** — inspect the ring terminal stack-to-post contact. Clean all surfaces, check stacking order, torque to spec.
2. **Cable 200 and its termination** — trace Cable 200 from battery negative to wherever it terminates. Inspect both ends. This cable bridges the structural ground to the battery — a bad connection at either end elevates the entire structural ground network.
3. **Structural ground connections in the battery area** — document ALL ground connections visible (bonding straps, frame joints, ground studs). Any metal-to-metal joint in the structural ground chain is a potential failure point.
4. **Pin 47 (wire 31299A22BL)** — trace from P701 Pin 47 to its termination. If it connects to a structural ground point, that point is in the return path through Cable 200 to battery negative.

**The ECU proves the dedicated-wire ground is healthy:** Per AMM p1936-1937 (Drawing D44-9274-10-00, EECU Wiring), the AE300 ECU grounds to **GS-IP-3/4** → GS-IP bus bar → wire 24008A4N → battery negative. This dedicated copper path reads correctly (~27.8V). The structural ground path (used by Pin 47 and the alternator) is the suspect. The ECU is also on the **ECU BUS** (directly off BATT BUS through a 100A fuse) — it bypasses the Power Relay, MAIN TIE, Ess Tie Relay, and ESS TIE that the Essential Bus must go through.

**Areas to inspect (in order of priority):**

1. **Battery negative terminal, Cable 200, and structural grounds (aft fuselage)** — Per IPC (24-31), three connections at battery negative: wire 24008A4N (instrument panel), wire 24405A6N (EPU), and **Cable 200** (D44-2403-160-00, "Cable, Battery GND" — routes to structural/airframe ground). Also check the **BatteryMinder interface** (installed Sep 2024). This terminal has been disturbed during both R&Rs and battery replacement. **Cable 200 is the bridge between the structural ground network and the battery** — trace it to its termination and inspect both ends. Clean all ring terminal contact surfaces, verify stacking order, torque to spec with DC4 per AMM 24-31. **Document ALL ground connections visible in the battery area** — there may be bonding straps or ground points not shown on the IPC.

2. **Pin 47 (ANALOG IN 5 LO) Essential Bus ground** — wire 31299A22BL (shielded) connects to the low side of the Essential Bus per the G1000 wiring diagram (D44-9231-60-03). The Electrical System schematic (D44-9224-30-01X03) shows only a generic ground symbol — **Pin 47 likely connects to the structural ground network**, not to a named GS-IP stud. If so, its return path goes: wire 31299A22BL → structural ground point → airframe structure → Cable 200 → battery negative post. Every joint in that chain is a potential failure point. The wire must be physically traced from P701 Pin 47 to wherever it terminates. **Other Diamond variant AMM wiring diagrams explicitly specify a ground stud (e.g. GS-IP-X) for this pin — the DA40 NG schematic does not.**
...
4. **GS-IP-14 / Pin 20 (POWER GROUND)** — wire 77016A22N from P701 Pin 20 to GS-IP-14. The GEA's power ground goes through the **dedicated wire path** (GS-IP-14 → bus bar → 24008A4N), proven healthy by ECU data. However, if Pin 20 (dedicated wire ground) and Pin 47 (structural ground) are at different potentials, the ADC common-mode range could be exceeded, causing the erratic readings seen in the flight data.
...
4. The **dedicated-wire GS-IP ground path is healthy** — ECU uses wire 24008A4N and reads correctly; the **structural ground** (used by Pin 47 and alternator) is the suspect
5. **Pin 47's ground termination must be traced** — the generic ground symbol means it likely connects to structural ground, not a GS-IP stud
6. **The battery negative terminal and Cable 200 must be inspected** — this is where the structural ground network meets the battery, and where the GPU bypass operates
...
└── output/
    ...
    ├── ecu_differential_three_period.png   # ECU differential three-period time series
    ├── ecu_differential_distributions.png  # ECU differential box plots + histograms
    └── ...
```
...
Run the ECU differential three-period analysis (requires `data/source/` CSVs and AustroView parsed ECU data):
