...
**Why the GPU test reads nearly correctly — current reversal at a bad connection:** Per IPC (24-31 Battery Installation), the battery negative terminal has only two connections: wire 24008A4N (4 AWG, to instrument panel) and wire 24405A6N (6 AWG, to EPU plug). The GPU negative connects to the **same battery negative terminal** — there is no separate GS-RP stud. Both tests share the same negative terminal and the same positive path from BATT BUS to the Essential Bus.

The key insight is that current flows in **opposite directions** through the battery negative terminal depending on the power source:
- **Battery only:** Current flows OUT of the battery, through the buses, and RETURNS through wire 24008A4N to the battery negative terminal. If there's resistance at the terminal (say 0.05Ω), the GS-IP bus bar sits ABOVE true battery negative by I×R (e.g. 20A × 0.05Ω = 1.0V). Pin 47 (voltage sense ground) references GS-IP, so it sits high → GEA reads **low**.
- **GPU connected:** The GPU charges the battery — current flows INTO the battery positive and OUT of the battery negative toward the buses. The current through the bad connection at the terminal now flows in the **opposite direction**. The I×R drop reverses polarity: GS-IP sits BELOW the battery negative terminal. Pin 47 sits low → GEA reads **slightly high**, canceling the error.

The same bad connection produces opposite effects: it makes the reading **low** on battery (return current) and **correct or slightly high** on GPU (charging current). The GPU doesn't fix the fault — it **reverses** its effect. This physics-based explanation accounts for the dramatic difference between -1.3V (battery) and -0.19V (GPU) on the same day without invoking intermittency, and puts the battery negative terminal back as a primary suspect for a significant portion of the total offset.
...
1. **Battery negative terminal (aft fuselage)** — Per IPC (24-31), only two wires connect here: 24008A4N (instrument panel) and 24405A6N (EPU). Also check the BatteryMinder interface (installed Sep 2024) connection. This terminal has been disturbed during both engine R&Rs and the battery replacement. Clean, check for corrosion, verify torque.
